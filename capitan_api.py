...
FIXED: Session refresh on token expiry - automatic token regeneration & transaction tracking
"""
...
# ================================================================
# SECTION 6: SECURITY & JWT AUTH (FIXED)
# ================================================================
...
def get_session(request: Request):
    ...
    # If token validation failed, decode unsafe payload to pull session identity
    if not payload:
        ...
        try:
            with get_db() as conn:
                with conn.cursor() as c:
                    ...
                    row = c.fetchone()
                    if row:
                        logger.info(f"Refreshed expired token context for session: {session_id}")
                        return {"id": row[0], "tier": row[1], "msg_count": row[2] or 0, "msg_window": row[3], "regenerate_token": True}
        except Exception as e:
            logger.error(f"Database error in get_session recovery block: {e}")
        return None
        ...
        with get_db() as conn:
            with conn.cursor() as c:
                ...
                if not row:
                    ...
                    conn.commit()
                    return {"id": session_id, "tier": tier, "msg_count": 0, "msg_window": now, "regenerate_token": false}
                
                if tier != row[1]:
                    c.execute("UPDATE sessions SET tier=%s, updated=%s WHERE id=%s", (tier, now, session_id))
                    conn.commit()
                    return {"id": row[0], "tier": tier, "msg_count": row[2] or 0, "msg_window": row[3], "regenerate_token": false}
                
                return {"id": row[0], "tier": row[1], "msg_count": row[2] or 0, "msg_window": row[3], "regenerate_token": False}
    except Exception as e:
...
🔬 GENERAL KNOWLEDGE:
- Physics, Chemistry, Biology, Medicine, History, Philosophy, Geopolitics.
...
def call_ai_fast(messages, tier="free"):
    ...
    if settings.OPENAI_KEY:
        try:
            r = requests.post("[https://api.openai.com/v1/chat/completions](https://api.openai.com/v1/chat/completions)",
...
app.add_middleware(
    ...
    allow_methods=["*"],
    allow_headers=["*"],
    expose_headers=["Authorization", "X-Refreshed-Token"]
)
...
def get_or_create_session(request: Request, response: Response):
    ...
    if session:
        ...
        return {**session, "token": token, "regenerated": True}
...
UPGRADE_BENEFITS = {
    ...
    "pro": ["Unlimited messages", "Deep AI (Claude Sonnet 4 / GPT-4o)", "Work Area (20 seats)", "Live market data", "Financial news"]
}
...
# ================================================================
# SECTION 23: MAIN CHAT ENDPOINT (FIXED)
# ================================================================
...
def chat(req: ChatRequest, request: Request, response: Response):
    ...
    # 🔑 CRITICAL FIX: If token context flagged it as expired, inject the updated token directly into Response Header
    if s.get("regenerate_token"):
        new_token = create_jwt(s["id"], s["tier"])
    ...
    return {
        ...
        "domain": domain,
        "token": new_token if s.get("regenerate_token") else None
    }
...
async def icon_192():
    svg = '''<svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" viewBox="0 0 100 100">
        ...
        <script>
            ...
            async function sendMessage() {
                ...
                try {
                    ...
                    // 🔑 Check response headers for newly refreshed tokens explicitly!
                    const refreshedToken = res.headers.get('X-Refreshed-Token');
                    if (refreshedToken) {
                        localStorage.setItem('cap_token', refreshedToken);
                        currentToken = refreshedToken;
                        console.log("Token updated seamlessly via interceptor context.");
                    }
