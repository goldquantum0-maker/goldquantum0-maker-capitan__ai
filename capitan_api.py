"""
CAPITAN AI — Enterprise Backend v31.1 (Productivity Edition)
CLOSEAI Technologies — CEO Osinachi Chukwu
World‑Class General‑Purpose Intelligence | Trustworthy | Self‑Learning | Safe
All features intact – base64 icons, correct manifest, research hub, recent chats fix, no missing components.
"""
    ...
    try:
        with get_db() as conn:
            with conn.cursor() as c:
                ...
                ''')
                for col, dtype in [("folder", "TEXT DEFAULT 'General'"), ("tags", "JSONB DEFAULT '[]'"), ("attachments", "JSONB DEFAULT '[]'"), ("pinned", "BOOLEAN DEFAULT FALSE"), ("updated", "TIMESTAMP DEFAULT NOW()")]:
                ...
                conn.commit()
        logger.info("✅ Database initialized (v31.1)")
...
# ================================================================================
# SYSTEM PROMPT (COMPLETE, NO CUTS)
# ================================================================================
...
def get_portfolio(request: Request, user: dict = Depends(get_current_user)):
    ...
    with get_db() as conn:
        with conn.cursor() as c:
            c.execute("SELECT id, name, content, folder, tags, attachments, pinned, created, updated FROM library_items WHERE user_id=%s ORDER BY pinned DESC, updated DESC", (user["id"],))
            items = []
            for row in c.fetchall():
                items.append({"id": row[0], "name": row[1], "content": row[2], "folder": row[3] or "General",
                              "tags": row[4] if row[4] else [], "attachments": row[5] if row[5] else [],
                              "pinned": row[6], "created": row[7].isoformat() if row[7] else None,
                              "updated": row[8].isoformat() if row[8] else None})
            return {"items": items}
...
def create_portfolio_item(req: PortfolioItemCreate, user: dict = Depends(get_current_user)):
    ...
    with get_db() as conn:
        with conn.cursor() as c:
            c.execute("INSERT INTO library_items (id, user_id, name, content, folder, tags, attachments) VALUES (%s,%s,%s,%s,%s,%s,%s)",
                      (item_id, user["id"], req.name, req.content, req.folder, json.dumps(req.tags), json.dumps(req.attachments)))
            conn.commit()
...
def update_portfolio_item(item_id: str, req: PortfolioItemCreate, user: dict = Depends(get_current_user)):
    ...
    with get_db() as conn:
        with conn.cursor() as c:
            c.execute("UPDATE library_items SET name=%s, content=%s, folder=%s, tags=%s, attachments=%s, updated=NOW() WHERE id=%s AND user_id=%s",
                      (req.name, req.content, req.folder, json.dumps(req.tags), json.dumps(req.attachments), item_id, user["id"]))
            conn.commit()
    return {"updated": True}
...
# built‑in topics management (founder only)
...
@app.get("/")
async def root():
    return {"name": "CAPITAN AI", "version": "31.1", "edition": "Productivity"}

if __name__ == "__main__":
    port = int(os.environ.get("PORT", 8000))
    print(f"\n{'='*70}")
    print(f"🚀 CAPITAN AI v31.1 — Productivity Edition")
