...
def get_chats(request: Request):
    ...
    if user:
        with get_db() as conn:
            with conn.cursor() as c:
                c.execute("SELECT id, title, topic_thread, created, updated FROM chats WHERE user_id=%s ORDER BY updated DESC LIMIT 50", (user["id"],))
                rows = c.fetchall()
        ...
        try:
            ...
            with get_db() as conn:
                with conn.cursor() as c:
                    c.execute("SELECT id, title, topic_thread, created, updated FROM chats WHERE session_id=%s ORDER BY updated DESC LIMIT 50", (session["id"],))
                    rows = c.fetchall()
                    return {"chats": [{"id": r[0], "title": r[1] or "New Chat", "topic": r[2], "created": r[3].isoformat() if r[3] else None, "updated": r[4].isoformat() if r[4] else None} for r in rows]}
        except: pass
    return {"chats": []}
...
async def manifest():
  ...
  "description": "Capitan AI is your intelligent companion for conversations, learning, productivity, and creativity. Ask questions, generate ideas, solve problems, write content, and receive thoughtful responses in natural language. Designed for smooth, human-like interaction, Capitan AI helps you stay informed, productive, and inspired wherever you are.",
  "dir": "auto",
  "lang": "en",
  "orientation": "any",
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*70}")
    print(f"🚀 CAPITAN AI v31.0 — Safety & Research Edition")
