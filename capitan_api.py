    ...
    try:
        with get_db() as conn:
            with conn.cursor() as c:
                ...
                c.execute('''CREATE TABLE IF NOT EXISTS uploaded_files (id TEXT PRIMARY KEY, session_id TEXT, filename TEXT, original_name TEXT, size INTEGER, mime_type TEXT, created TIMESTAMP)''')
...
def create_jwt(session_id, tier):
    ...
    s = base64.urlsafe_b64encode(hmac.new(settings.JWT_SECRET.encode(),f"{h}.{p}".encode(),hashlib.sha256).digest()).decode().rstrip("=")
    return f"{h}.{p}.{s}"
...
def verify_jwt(token):
    try:
        ...
        h,p,s = parts
        es = base64.urlsafe_b64encode(hmac.new(settings.JWT_SECRET.encode(),f"{h}.{p}".encode(),hashlib.sha256).digest()).decode().rstrip("=")
...
def classify(q):
    ...
    if re.search(r'crispr|dna|rna|protein|cell|gene|genome|physics|quantum|chemistry|biology|neuroscience|climate|energy|health|medicine|disease|symptom|treatment|diagnosis',q): return 'science'
    if re.search(r'```|def |class |import |from |package|npm|pip|docker|kubernetes|aws|api|rest|graphql|sql|database|query|react|node|javascript|typescript|python|rust|golang',q): return 'coding'
    if re.search(r'stochastic|ito|black.scholes|monte carlo|var|cvar|sharpe|sortino|beta|alpha|option pricing|derivative|risk neutral|fama|french|cointegration|garch|arima|backtest|factor model',q): return 'quant'
    if re.search(r'dcf|discounted cash flow|ebitda|ebit|revenue|earnings|balance sheet|income statement|cash flow|valuation|wacc|capm|pe ratio|pb ratio|ev/ebitda|dividend|yield|bond|coupon|duration|convexity|forex|fx|central bank|federal reserve|interest rate|inflation|gdp|macro|equity|stock|market|trading|invest|portfolio|crypto|bitcoin|ethereum|defi|ngx|jse|gse|african market|gold|silver|oil|commodity',q): return 'finance'
    if re.search(r'prove|proof|theorem|lemma|corollary|derive|integral|derivative|differential equation|linear algebra|matrix|eigenvalue|vector|topology|group theory|probability|statistics',q): return 'math'
    return 'general'

# ================================================================
# AI SERVICE (MULTI-PROVIDER WITH FULL CAPABILITIES)
# ================================================================
        ...
        try:
            ...
            r = requests.post("https://api.groq.com/openai/v1/chat/completions",
                ...
                json={"model":"llama-3.3-70b-versatile" if tier in ("pro","founder") else "llama-3.1-8b-instant",
                      "messages":groq_msgs,"temperature":0.5,"max_tokens":2000 if tier in ("pro","founder") else 800},timeout=25)
            if r.status_code==200:
                content = r.json().get("choices",[{}])[0].get("message",{}).get("content","")
                if content: return content, "groq/llama-3.3-70b" if tier in ("pro","founder") else "groq/llama-3.1-8b"
        except: pass
    
    # OpenRouter (Claude/GPT-4 for Pro/Founder)
...
def system_prompt(domain, tier, session_id=None, web_results=None):
    ...
    base = base.replace("{time_of_day}", tc["time_of_day"]).replace("{greeting_context}", tc["greeting_context"])
...
def health():
    ...
    if ai_status != "connected" and settings.OPENROUTER_KEY:
        try:
            r = requests.post("https://openrouter.ai/api/v1/chat/completions",
                headers={"Authorization":f"Bearer {settings.OPENROUTER_KEY}","Content-Type":"application/json"},
    ...
    return {"status":"ok","version":"25.0","ai":ai_status,"providers":providers,"database":db_status}
...
UPGRADE_BENEFITS = {
    "plus": ["40 messages per day","Smart AI model","Work Area (7 seats)","File uploads (10MB)","Web search","Coding & Quant tools"],
    "pro": ["Unlimited messages","Deep AI (Claude/GPT-4)","Work Area (20 seats)","Live market data","Financial news","Web search","Business mode"]
}
...
def tech_news(request: Request):
    s = get_session(request)
    if not s or s["tier"] not in ("pro","founder"): return {"news":[],"message":"Pro tier required"}
...
# ================================================================
# MAIN CHAT ENDPOINT (FULL INTELLIGENCE)
# ================================================================
...
def chat(req: ChatRequest, request: Request):
    s = get_session(request)
    if not s: raise HTTPException(401)
    if not check_rate(s["id"], s["tier"]): raise HTTPException(429, "Rate limit exceeded")
...
async def upload_file(request: Request, file: UploadFile = File(...)):
    ...
    try:
        with get_db() as conn:
            with conn.cursor() as c:
                c.execute("INSERT INTO uploaded_files (id,session_id,filename,original_name,size,mime_type,created) VALUES (%s,%s,%s,%s,%s,%s,%s)",
                         (file_id, s["id"], file_id, file.filename or "unknown", len(contents), file.content_type or "application/octet-stream", datetime.utcnow()))
                conn.commit()
    except: pass
...
if __name__ == "__main__":
    ...
    print(f"🚀 CAPITAN AI v25.0 - Full Intelligence")
    print(f"📊 Database: {'Supabase' if settings.DATABASE_URL or settings.SUPABASE_DB_PASSWORD else 'SQLite Fallback'}")
    print(f"🤖 AI: Groq={bool(settings.GROQ_KEY)} | OpenRouter={bool(settings.OPENROUTER_KEY)}")
