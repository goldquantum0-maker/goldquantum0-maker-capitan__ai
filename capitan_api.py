...
def get_current_user(request: Request):
    ...
    try:
        with get_db() as conn:
            with conn.cursor() as c:
                c.execute("SELECT id, telegram_id, telegram_username, first_name, last_name, name, tier, created_at FROM users WHERE id = %s", (user_id,))
                row = c.fetchone()
                if row:
                    name = row[5] or row[3] or row[2] or f"User_{row[1]}"
                    return {
                        "id": row[0],
                        "telegram_id": row[1],
...
def founder(req: FounderRequest, user: dict = Depends(get_current_user)):
    ...
    try:
        with get_db() as conn:
            with conn.cursor() as c:
                c.execute("UPDATE users SET tier = 'founder', updated_at = NOW() WHERE id = %s", (user["id"],))
                conn.commit()
    except Exception as e:
